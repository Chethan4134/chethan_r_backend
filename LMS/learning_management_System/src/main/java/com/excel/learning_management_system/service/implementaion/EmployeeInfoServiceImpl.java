...
import com.excel.learning_management_system.repository.ExperienceRepository;
...
//----------------------------------------------------------------------------------------------------------------------
	...
	public String addContactDetails(EmployeeContactList dto) {
		...
		if (optional.isPresent()) {
			...
			EmployeePrimaryInfo employeePrimaryInfo = optional.get();
			List<EmployeeContactInfo> contactInfos = EmployeeUtil.dtoToContactEntity(dto.getContactList());
...
//----------------------------------------------------------------------------------------------------------------------
	...
	public String addExpirenceDetails(EmployeeExperienceList dto) {
		...
		if(optional.isPresent()) {
			...
			List<EmployeeExperienceInfo> experienceInfos = EmployeeUtil.dtoToExperienceEntity(dto.getEmployeeExperience());
			
			if(employeePrimaryInfo.getEmployeeExperience() != null) {
				employeePrimaryInfo.getEmployeeExperience().forEach(experienceRepository::delete);
			}
			
			experienceInfos.stream().forEach(exp -> exp.setEmployee(employeePrimaryInfo));
			employeePrimaryInfo.setEmployeeExperience(experienceInfos);
			employeeRepository.save(employeePrimaryInfo);
